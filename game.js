...
async function startOnlineGame(){
  ...
  G.players.forEach(p=>{
    if(p.coins===undefined)p.coins=7;
...
function startGame(n){
  ...
  const players=Array.from({length:n},(_,i)=>({id:i,name:names[i],isHuman:i===0,hand:[],coins:7}));
  players.forEach(p=>p.coins-=1);
  G={
    online:false,
    phase:'reveal',
    players,
...
function doShowdown(){
  ...
  winners.forEach(w=>{w.p.coins=(w.p.coins||0)+share;});
  if(potWon%winners.length>0)winners[0].p.coins++;
  G.pot=0;
  ...
  if(!G.online){
    const stillIn=G.players.filter(p=>(p.coins||0)>0);
    if(stillIn.length<=1){
      const gw=stillIn[0];
...
function startNextRound(){
  ...
  G.players=G.players.filter(p=>(p.coins||0)>0);
  ...
  // Ante
  G.pot=0;
  G.players.forEach(p=>{const ante=Math.min(1,(p.coins||0));p.coins=(p.coins||0)-ante;G.pot+=ante;});
  // Re-deal
  const deck=shuffle(mkDeck());
...
function processBetAction(pi,action,amount=0){
  ...
  if(action==='check'){
    ...
    const pay=Math.min(bs.currentBet,(p.coins||0));
    p.coins=(p.coins||0)-pay;G.pot=(G.pot||0)+pay;
    bs.calledIdx.push(pi);
  }else if(action==='raise'){
    const active=bs.order.filter(i=>!bs.foldedIdx.includes(i));
    const maxRaise=Math.max(...active.map(i=>(G.players[i].coins||0)));
    const raise=Math.min(Math.max(1,amount),maxRaise);
    const pay=Math.min(raise,(p.coins||0));
    p.coins=(p.coins||0)-pay;G.pot=(G.pot||0)+pay;
    bs.currentBet=raise;
    bs.calledIdx=[pi];
  }else if(action==='fold'){
...
function aiBettingTurn(pi){
  ...
  else if((G.players[pi].coins||0)<=0){processBetAction(pi,'fold');}
...
function oppBoxHTML(p,active,pos){
  ...
  const coinsDisp=p.coins!==undefined?`<div class="opp-coins">💰${p.coins}</div>`:'';
...
function renderPlayer(){
  ...
  const coinsStr=p.coins!==undefined?`　／　コイン: ${p.coins}枚`:'';
...
function renderActions(){
  ...
  if(G.phase==='reveal'){
    ...
    if(G.online&&!isMyTurn()){
      ...
      const round=Math.floor(G.turnsDone/G.players.length)+1;
      const cantDraw=G.deck.length===0||(round===2&&(myPlayer().coins||0)<=0);
      const drawLabel=round===2?'① カードを引く (-1コイン)':'① カードを引く';
    ...
    if(G.online&&G.playerIds[pi]!==myPlayerId){
      ...
      const p=myPlayer();
      const active=bs.order.filter(i=>!bs.foldedIdx.includes(i));
      const maxRaise=active.length>0?Math.max(...active.map(i=>(G.players[i].coins||0))):0;
      h=`<button class="btn btn-cancel" onclick="bettingFold()">フォールド</button>
      ${bs.currentBet===0
        ?`<button class="btn btn-ok" onclick="bettingCheck()">チェック</button>`
        :`<button class="btn btn-ok" onclick="bettingCall()" ${(p.coins||0)>0?'':'disabled'}>コール (${Math.min(bs.currentBet,(p.coins||0))}コイン)</button>`}
