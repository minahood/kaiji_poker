...
function chipLogDisp(p){
  ...
  return`<div class="chip-log-wrap" onmouseenter="showChipTip(event,'${logLines}')" onmouseleave="hideChipTip()"><span class="chip-log-label">消費</span><img src="${CHIPS_URL}" class="chip-img" alt="チップ"><span class="chip-log-count">×${total}</span></div>`;
}
...
function selectDraw(){
  ...
  G.phase='human-draw';
  G.msg='捨てるカードをクリックしてください。（開示カード：無料、非開示：1チップ）';
  render();
}
...
function confirmDiscard2(){
  ...
  if(revIdxs.length===0||G.deck.length<revIdxs.length)return;
  const round=Math.floor(G.turnsDone/G.players.length)+1;
  if(round===2){p.chips=Math.max(0,(p.chips||0)-1);G.pot=(G.pot||0)+1;}
  flyOutCards(revIdxs);
  [...revIdxs].sort((a,b)=>b-a).forEach(i=>p.hand.splice(i,1));
  const newCards=revIdxs.map(()=>{const nc=G.deck.shift();nc.revealed=true;return nc;});
  newCards.forEach(nc=>p.hand.push(nc));
  pendingDealIn=newCards.map((_,k)=>p.hand.length-newCards.length+k);
  G.msg=`開示カード${revIdxs.length}枚を全て交換しました。`;
  G.phase='turn-end';
  if(G.online){pushState().then(()=>{render();setTimeout(endTurn,1400);});}
  else{render();setTimeout(endTurn,1400);}
}
...
function orderedOpponents(){
  const me=myPlayer();
  const myPos=G.turnOrder.indexOf(myIdx);
  const result=[];
  for(let i=1;i<G.turnOrder.length;i++){
    const pi=G.turnOrder[(myPos+i)%G.turnOrder.length];
    const p=G.players[pi];
    if(G.online?p.id!==me.id:!p.isHuman)result.push(p);
  }
  return result;
}
...
function renderModal(){
  ...
  if(ts.phase==='pick'){
    ...
    let h='';
    if(opps.length>1){
      h+=`<div class="ms"><label>交換する相手：</label><div>`;
      opps.forEach(p=>{
        h+=`<button class="tgt-btn${ts.targetIdx===p.id?' on':''}" onclick="pickTarget(${p.id})">${p.name}</button>`;
      });
      h+=`</div></div>`;
    }
    h+=`<div class="ms"><label>提示するカード（1枚以上）：</label><div class="incoming-hand" id="mc"></div></div>`;
...
function aiTurn(pi){
  ...
  const discIsRevealed=p.hand[discIdx]&&p.hand[discIdx].revealed;
...
function flipBeforeShowdown(cb){
  ...
  // フォールドしていない・操作プレイヤー以外のみめくる
  const playerCards=activePlayers.filter(p=>p.id!==myIdx).map(p=>({
    pi:p.id,
...
function renderPlayer(){
  ...
  const isTradePickPhase=G.phase==='human-trade-select';
  let row1='',row2='';
  if(isRevPhase){
    p.hand.forEach((c,i)=>{
    ...
    const tradeNonRev=p.hand.map((c,i)=>({c,i})).filter(x=>!x.c.revealed)
      .sort((a,b)=>a.c.value-b.c.value||SUIT_STR[b.c.suit]-SUIT_STR[a.c.suit]);
    const tradeRev=p.hand.map((c,i)=>({c,i})).filter(x=>x.c.revealed);
    tradeRev.forEach(x=>{row2+=cardHTML(x.c,{click:true,sel:ts.offerIdx.includes(x.i),fn:'toggleOffer',idx:x.i});});
    tradeNonRev.forEach(x=>{row1+=cardHTML(x.c,{click:true,sel:ts.offerIdx.includes(x.i),fn:'toggleOffer',idx:x.i});});
    document.getElementById('player-hand').innerHTML=
      `<div class="hand-row rev-row">${row2}</div>`+
...
function renderActions(){
  ...
  if(G.phase==='reveal'){
    ...
    if(G.online&&!isMyTurn()){
      ...
      const round=Math.floor(G.turnsDone/G.players.length)+1;
      const cantDraw=G.deck.length===0;
      h=`<button class="btn btn-draw" onclick="selectDraw()" ${cantDraw?'disabled':''}>① カードを引く</button>
      <button class="btn btn-trade" onclick="selectTrade()">② カードの交換を提案</button>`;
    }
    ...
    if(G.online&&G.playerIds[pi]!==myPlayerId){
      ...
      const p=myPlayer();
      const active=bs.order.filter(i=>!bs.foldedIdx.includes(i));
      const others=active.filter(i=>i!==myIdx);
      const maxOther=others.length>0?Math.max(...others.map(i=>(G.players[i].chips||0))):0;
      const maxRaise=Math.min(p.chips||0,maxOther);
    ...
    h=`<span style="color:#8a7a6a;font-size:.9rem;letter-spacing:.05em">捨てるカードをクリック（開示：無料 / 非開示：1チップ）</span>
    <button class="btn btn-cancel" onclick="cancelDraw()">キャンセル</button>`;
  }else if(G.phase==='human-trade-select'){
    const ts=G.tradeState||{targetIdx:null,offerIdx:[]};
