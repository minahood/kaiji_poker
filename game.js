...
function confirmReveal(){
  ...
  G.selCards.forEach(i=>{G.players[0].hand[i].revealed=true;});
...
function proposeToAI(){
  ...
  setTimeout(()=>{
    const ai=G.players[ts.targetIdx];
    const offered=ts.offerIdx.map(i=>G.players[0].hand[i]);
...
function execTrade(){
  ...
  const h=G.players[0],ai=G.players[ts.targetIdx];
...
function renderIncomingGive(){
  const ts=G.tradeState;
  let ch=G.players[0].hand.map((c,i)=>
...
function execIncoming(){
  const ts=G.tradeState;
  const h=G.players[0],ai=G.players[ts.proposerIdx];
...
function aiTurn(pi){
  const p=G.players[pi];
  if(G.deck.length>0&&Math.random()<0.7){
    const discIdx=p.hand.map((c,i)=>({c,i})).sort((a,b)=>a.c.value-b.c.value)[0].i;
    ...
    if(nonRev.length>0){
      ...
      if(G.deck.length>0){
        const di=p.hand.map((c,i)=>({c,i})).sort((a,b)=>a.c.value-b.c.value)[0].i;
...
function renderOpponents(){
  ...
  G.players.filter(p=>!p.isHuman).forEach(p=>{
    ...
    const active=p.id===pi&&['ai-turn','turn-end'].includes(G.phase);
    zones[pos]+=oppBoxHTML(p,active,pos);
  });
